...
if (__name__ == "__main__"):
    ...
    for i in range(noise_class.method_nb) :
        ...
        for j in range(denoise_class.method_nb) :
            ...
            I_imwrite = (im*255).astype(np.uint8)
            cv2.imwrite('results_expansion/'+str(i)+'_'+str(j)+'.png', I_imwrite)

            I_imwrite2 = (im_denoise*255).astype(np.uint8)
            cv2.imwrite('results_expansion/'+str(i)+'_'+str(j)+'.png', I_imwrite2)

            I_imwrite3 = (evaluation.Idiff*255).astype(np.uint8)
            cv2.imwrite('results_expansion/'+str(i)+'_'+str(j)+'.png', I_imwrite3)
